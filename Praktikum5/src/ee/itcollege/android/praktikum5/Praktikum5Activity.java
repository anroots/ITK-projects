...
public class Praktikum5Activity extends FragmentActivity implements OnTweetClickedListener {
	...
	public void onTweetClicked(Tweet tweet) {
		TweetAuthorFragment fragment = (TweetAuthorFragment) getSupportFragmentManager()
				.findFragmentById(R.id.fragment_tweet_author);
		if (fragment != null && fragment.isVisible()) {
			fragment.showTweetsFromUser(tweet.getAuthorName(), tweet.getAuthorId());
		} else {
			
			Intent intent = new Intent(this,Praktikum5AuthorActivity.class);
			intent.putExtra("authorName", tweet.getAuthorName());
			intent.putExtra("authorId", tweet.getAuthorId());
